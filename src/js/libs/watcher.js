...
import { FLS, isMobile, uniqArray } from '../files/functions.js';
...
// animate__animated - класс который добавляется при появлении объекта
...
class ScrollWatcher {
      ...
      let uniqParams = uniqArray(
        Array.from(items).map(function (item) {
          return `${item.dataset.watchRoot ? item.dataset.watchRoot : null}|${
            item.dataset.watchMargin ? item.dataset.watchMargin : '0px 0px -120px 0px'
  ...
  scrollWatcherIntersecting(entry, targetElement) {
    if (entry.isIntersecting) {
      // Видим объект
      // Добавляем класс
      !targetElement.classList.contains('animate__animated')
        ? targetElement.classList.add('animate__animated')
        : null;
      this.scrollWatcherLogging(
        `Я вижу ${targetElement.classList}, добавил класс animate__animated`
      );
    } else {
      // Не видим объект
      // Убираем класс
      targetElement.classList.contains('animate__animated')
        ...
        : null;
      this.scrollWatcherLogging(
        `Я не вижу ${targetElement.classList}, убрал класс animate__animated`
      );
    }
  }
  ...
  scrollWatcherCallback(entry, observer) {
    ...
    if (entry.isIntersecting) {
      ...
      target.classList.add(`animate__${target.dataset.watch}`);
    }
  }
}
